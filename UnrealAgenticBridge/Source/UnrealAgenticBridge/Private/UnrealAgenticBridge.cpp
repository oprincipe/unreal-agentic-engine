...
UUnrealAgenticBridge::UUnrealAgenticBridge()
{
...
void UUnrealAgenticBridge::Initialize(FSubsystemCollectionBase& Collection)
{
    UE_LOG(LogTemp, Display, TEXT("UnrealAgenticBridge: Initializing"));
...
void UUnrealAgenticBridge::Deinitialize()
{
    UE_LOG(LogTemp, Display, TEXT("UnrealAgenticBridge: Shutting down"));
    StopServer();
}

// Start the MCP server
void UUnrealAgenticBridge::StartServer()
{
    if (bIsRunning)
    {
        UE_LOG(LogTemp, Warning, TEXT("UnrealAgenticBridge: Server is already running"));
        return;
    }
    ...
    if (!SocketSubsystem)
    {
        UE_LOG(LogTemp, Error, TEXT("UnrealAgenticBridge: Failed to get socket subsystem"));
        return;
    }

    // Create listener socket
    TSharedPtr<FSocket> NewListenerSocket = MakeShareable(SocketSubsystem->CreateSocket(NAME_Stream, TEXT("UnrealAgenticBridgeListener"), false));
    if (!NewListenerSocket.IsValid())
    {
        UE_LOG(LogTemp, Error, TEXT("UnrealAgenticBridge: Failed to create listener socket"));
        return;
    }
    ...
    {
        UE_LOG(LogTemp, Error, TEXT("UnrealAgenticBridge: Failed to bind listener socket to %s:%d"), *ServerAddress.ToString(), Port);
        return;
    }
    ...
    {
        UE_LOG(LogTemp, Error, TEXT("UnrealAgenticBridge: Failed to start listening"));
        return;
    }

    ListenerSocket = NewListenerSocket;
    bIsRunning = true;
    UE_LOG(LogTemp, Display, TEXT("UnrealAgenticBridge: Server started on %s:%d"), *ServerAddress.ToString(), Port);

    // Start server thread
    ServerThread = FRunnableThread::Create(
        new FMCPServerRunnable(this, ListenerSocket),
        TEXT("UnrealAgenticBridgeServerThread"),
        0, TPri_Normal
    );

    if (!ServerThread)
    {
        UE_LOG(LogTemp, Error, TEXT("UnrealAgenticBridge: Failed to create server thread"));
        StopServer();
        return;
    }
}
...
// Stop the MCP server
void UUnrealAgenticBridge::StopServer()
{
    if (!bIsRunning)
    {
        return;
    ...
    UE_LOG(LogTemp, Display, TEXT("UnrealAgenticBridge: Server stopped"));
}

// Execute a command received from a client
FString UUnrealAgenticBridge::ExecuteCommand(const FString& CommandType, const TSharedPtr<FJsonObject>& Params)
{
    UE_LOG(LogTemp, Display, TEXT("UnrealAgenticBridge: Executing command: %s"), *CommandType);
    ...
    AsyncTask(ENamedThreads::GameThread, [this, CommandType, Params, Promise = MoveTemp(Promise)]() mutable
    {
        ...
        try
        {
                     ...
                     CommandType == TEXT("test_connection"))
            {
                ...
                UE_LOG(LogTemp, Warning, TEXT("UnrealAgenticBridge: chat_message received but Python server must be running to handle it."));
            }
