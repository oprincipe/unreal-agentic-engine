...
static EUnrealAgenticBridgeProvider          GModelOptionsProvider = EUnrealAgenticBridgeProvider::Anthropic;

static TArray<FString> GetDefaultModelsForProvider(EUnrealAgenticBridgeProvider Provider)
{
    switch (Provider)
    {
    case EUnrealAgenticBridgeProvider::Anthropic:
        return { TEXT("claude-opus-4-5"), TEXT("claude-sonnet-4-5"),
                 TEXT("claude-3-5-sonnet-latest"), TEXT("claude-3-5-haiku-latest") };
    case EUnrealAgenticBridgeProvider::OpenAI:
        return { TEXT("gpt-4o"), TEXT("gpt-4o-mini"), TEXT("gpt-4-turbo"), TEXT("gpt-3.5-turbo") };
    case EUnrealAgenticBridgeProvider::Google:
        return { TEXT("gemini-2.0-flash"), TEXT("gemini-2.0-flash-lite"),
                 TEXT("gemini-1.5-pro"), TEXT("gemini-1.5-flash") };
    case EUnrealAgenticBridgeProvider::Ollama:
...
static void RebuildModelOptions(const EUnrealAgenticBridgeProvider Provider, const TArray<FString>& Models)
{
    GModelOptions.Empty();
...
TSharedRef<IDetailCustomization> FUnrealAgenticEditorSettingsCustomization::MakeInstance()
{
    return MakeShareable(new FUnrealAgenticEditorSettingsCustomization);
}

void FUnrealAgenticEditorSettingsCustomization::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
    CachedDetailBuilder = &DetailBuilder;

    const UUnrealAgenticEditorSettings* Settings = GetDefault<UUnrealAgenticEditorSettings>();
    const EUnrealAgenticBridgeProvider CurrentProvider = Settings ? Settings->Provider : EUnrealAgenticBridgeProvider::Anthropic;
    ...
    TSharedRef<IPropertyHandle> ProviderProp  = DetailBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(UUnrealAgenticEditorSettings, Provider));
    TSharedRef<IPropertyHandle> ModelNameProp = DetailBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(UUnrealAgenticEditorSettings, ModelName));
    TSharedRef<IPropertyHandle> ApiKeyProp    = DetailBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(UUnrealAgenticEditorSettings, ApiKey));
    TSharedRef<IPropertyHandle> OllamaUrlProp = DetailBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(UUnrealAgenticEditorSettings, OllamaServerUrl));

    ProviderProp->SetOnPropertyValueChanged(FSimpleDelegate::CreateLambda([this, ModelNameProp]()
    {
        const UUnrealAgenticEditorSettings* S = GetDefault<UUnrealAgenticEditorSettings>();
        if (S)
        {
    ...
    bool bIsOllama = (CurrentProvider == EUnrealAgenticBridgeProvider::Ollama);
    ...
    [
        ...
        [
            ...
            .OnClicked(this, &FUnrealAgenticEditorSettingsCustomization::OnRefreshModelsClicked)
        ]
    ];
    ...
    [
        ...
        .OnClicked(this, &FUnrealAgenticEditorSettingsCustomization::OnTestConnectionClicked)
    ];
}

// ─────────────────────────────────────────────────────────────────────────────
FReply FUnrealAgenticEditorSettingsCustomization::OnRefreshModelsClicked()
{
    const UUnrealAgenticEditorSettings* Settings = GetDefault<UUnrealAgenticEditorSettings>();
    if (!Settings) return FReply::Handled();

    const bool bIsOllama = (Settings->Provider == EUnrealAgenticBridgeProvider::Ollama);
    ...
    if (bIsOllama)
    {
        ...
        Req->OnProcessRequestComplete().BindLambda(
            [this](FHttpRequestPtr, const FHttpResponsePtr& Resp, const bool bOK)
            {
                ...
                RebuildModelOptions(EUnrealAgenticBridgeProvider::Ollama, Names);
                
                UUnrealAgenticEditorSettings* MutableSettings = GetMutableDefault<UUnrealAgenticEditorSettings>();
    ...
    else
    {
        ...
        EUnrealAgenticBridgeProvider Prov = S ? S->Provider : EUnrealAgenticBridgeProvider::Anthropic;
        ...
        Req->OnProcessRequestComplete().BindLambda(
            [this, Prov](FHttpRequestPtr, const FHttpResponsePtr& Resp, const bool bOK)
            {
                ...
                if (Prov == EUnrealAgenticBridgeProvider::Google)
                {
                    const TArray<TSharedPtr<FJsonValue>>* ModelsPtr = nullptr;
                    if (Json->TryGetArrayField(TEXT("models"), ModelsPtr) && ModelsPtr)
                    {
...
FReply FUnrealAgenticEditorSettingsCustomization::OnTestConnectionClicked()
{
    const UUnrealAgenticEditorSettings* Settings = GetDefault<UUnrealAgenticEditorSettings>();
    if (!Settings) return FReply::Handled();

    // ─── Ollama: test directly against the local REST API, no Python agent needed ───
    if (Settings->Provider == EUnrealAgenticBridgeProvider::Ollama)
    {
    ...
    {
        FUnrealAgenticModule::Get().EnsureAgentRunning();
    }

    const UEnum* ProviderEnum = StaticEnum<EUnrealAgenticBridgeProvider>();
...
EVisibility FUnrealAgenticEditorSettingsCustomization::GetApiKeyVisibility() const
{
    const UUnrealAgenticEditorSettings* Settings = GetDefault<UUnrealAgenticEditorSettings>();
    return (Settings && Settings->Provider == EUnrealAgenticBridgeProvider::Ollama)
        ? EVisibility::Collapsed : EVisibility::Visible;
}

EVisibility FUnrealAgenticEditorSettingsCustomization::GetOllamaUrlVisibility() const
{
    const UUnrealAgenticEditorSettings* Settings = GetDefault<UUnrealAgenticEditorSettings>();
    return (Settings && Settings->Provider == EUnrealAgenticBridgeProvider::Ollama)
