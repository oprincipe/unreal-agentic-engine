...
void FUnrealAgenticModule::StartupModule()
{
	...
	FGlobalTabmanager::Get()->RegisterNomadTabSpawner(
		FName("UnrealAgenticEngineChatTab"),
...
void FUnrealAgenticModule::ShutdownModule()
{
	...
	FGlobalTabmanager::Get()->UnregisterNomadTabSpawner(FName("UnrealAgenticEngineChatTab"));
...
void FUnrealAgenticModule::RegisterMenus()
{
	...
	FToolMenuSection& Section = Menu->FindOrAddSection("UnrealAgenticEngineSection");
	Section.AddMenuEntry(
		"OpenUnrealAgenticEngineChat",
		LOCTEXT("OpenChatLabel",   "Unreal AI Agent"),
		LOCTEXT("OpenChatTooltip", "Open the Unreal AI Agent chat window."),
		FSlateIcon(),
		FUIAction(FExecuteAction::CreateLambda([]() {
			FGlobalTabmanager::Get()->TryInvokeTab(FName("UnrealAgenticEngineChatTab"));
		}))
	);
}
...
void FUnrealAgenticModule::EnsureAgentRunning()
{
	...
	FString PluginDir = IPluginManager::Get().FindPlugin(TEXT("UnrealAgenticEngine"))->GetBaseDir();
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("UnrealAgenticEngine: Agent script not found at %s"), *ScriptPath);
	...
	FString SavedDir = FPaths::ConvertRelativePathToFull(
		FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("UnrealAgenticEngine"))
	);
	...
	// locate the correct [PROJECT]/Saved/UnrealAgenticEngine/ directory.
	...
	{
		UE_LOG(LogTemp, Display, TEXT("UnrealAgenticEngine: Launched Python Agent (PID %d)"), OutProcessId);
	}
	else
	{
		UE_LOG(LogTemp, Error, TEXT("UnrealAgenticEngine: Failed to launch Python. Is Python installed and in your PATH?"));
...
void FUnrealAgenticModule::StopAgent()
{
	if (AgentProcess.IsValid())
	{
		if (FPlatformProcess::IsProcRunning(AgentProcess))
		{
			...
			UE_LOG(LogTemp, Display, TEXT("UnrealAgenticEngine: Python Agent terminated."));
		}
...
IMPLEMENT_MODULE(FUnrealAgenticModule, UnrealAgenticEngine) 