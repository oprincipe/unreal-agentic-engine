...
static EUnrealAgenticEngineProvider          GModelOptionsProvider = EUnrealAgenticEngineProvider::Anthropic;

static TArray<FString> GetDefaultModelsForProvider(EUnrealAgenticEngineProvider Provider)
{
    switch (Provider)
    {
    case EUnrealAgenticEngineProvider::Anthropic:
        return { TEXT("claude-opus-4-5"), TEXT("claude-sonnet-4-5"),
                 TEXT("claude-3-5-sonnet-latest"), TEXT("claude-3-5-haiku-latest") };
    case EUnrealAgenticEngineProvider::OpenAI:
        return { TEXT("gpt-4o"), TEXT("gpt-4o-mini"), TEXT("gpt-4-turbo"), TEXT("gpt-3.5-turbo") };
    case EUnrealAgenticEngineProvider::Google:
        return { TEXT("gemini-2.0-flash"), TEXT("gemini-2.0-flash-lite"),
                 TEXT("gemini-1.5-pro"), TEXT("gemini-1.5-flash") };
    case EUnrealAgenticEngineProvider::Ollama:
...
static void RebuildModelOptions(const EUnrealAgenticEngineProvider Provider, const TArray<FString>& Models)
{
    GModelOptions.Empty();
...
void FUnrealAgenticEditorSettingsCustomization::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
    ...
    const EUnrealAgenticEngineProvider CurrentProvider = Settings ? Settings->Provider : EUnrealAgenticEngineProvider::Anthropic;
    ...
    bool bIsOllama = (CurrentProvider == EUnrealAgenticEngineProvider::Ollama);
...
FReply FUnrealAgenticEditorSettingsCustomization::OnRefreshModelsClicked()
{
    ...
    const bool bIsOllama = (Settings->Provider == EUnrealAgenticEngineProvider::Ollama);
    ...
    if (bIsOllama)
    {
        ...
        Req->OnProcessRequestComplete().BindLambda(
            [this](FHttpRequestPtr, const FHttpResponsePtr& Resp, const bool bOK)
            {
                ...
                RebuildModelOptions(EUnrealAgenticEngineProvider::Ollama, Names);
    ...
    else
    {
        ...
        EUnrealAgenticEngineProvider Prov = S ? S->Provider : EUnrealAgenticEngineProvider::Anthropic;
        ...
        Req->OnProcessRequestComplete().BindLambda(
            [this, Prov](FHttpRequestPtr, const FHttpResponsePtr& Resp, const bool bOK)
            {
                ...
                if (Prov == EUnrealAgenticEngineProvider::Google)
                {
                    const TArray<TSharedPtr<FJsonValue>>* ModelsPtr = nullptr;
                    if (Json->TryGetArrayField(TEXT("models"), ModelsPtr) && ModelsPtr)
                    {
...
FReply FUnrealAgenticEditorSettingsCustomization::OnTestConnectionClicked()
{
    ...
    if (Settings->Provider == EUnrealAgenticEngineProvider::Ollama)
    {
...
{
    const UUnrealAgenticEditorSettings* Settings = GetDefault<UUnrealAgenticEditorSettings>();
    return (Settings && Settings->Provider == EUnrealAgenticEngineProvider::Ollama)
